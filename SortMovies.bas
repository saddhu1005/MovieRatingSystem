...
Public Sub Sort_Current()
    ...
    Set rs = db.OpenRecordset("select * from MovieDetails where CURRENT=1 ORDER BY NAME ASC")
    rs.MoveLast
...
End Sub
Public Sub sort_rating()
 Dim db_path As String
    Dim db As Database
    Dim rs As Recordset
    Dim total_current_movies As Integer
    Form4.Label11.Caption = "MOVIES :   TOP RATED"
    Form4.Label11.FontUnderline = True
    db_path = App.Path + "\db\MovieRatingSystem.mdb"
    Set db = OpenDatabase(db_path)
    Set rs = db.OpenRecordset("select * from MovieDetails order by RATING DESC")
    rs.MoveLast
    total_current_records = rs.RecordCount
    rs.MoveFirst
    current_movies = rs.GetRows(total_current_records)
